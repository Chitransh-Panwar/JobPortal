    ...
    return (
        <div className='max-w-7xl mx-auto my-20 px-4'>
            ...
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-3 my-5'>
                {
                    jobsLoading ? <span className='text-violet-700 font-medium'>Loading jobs...</span> : allJobs.length <= 0 ? <span>No Job Available</span> : allJobs?.slice(0,6).map((job) => <LatestJobCards key={job._id} job={job}/>)
                }
            </div>
        </div>
    )
